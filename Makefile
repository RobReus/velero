...
DOCKER_PLATFORMS ?= linux/amd64,linux/arm64,linux/arm/v7,linux/ppc64le
...
build-image:
	...
	cd hack/build-image && docker build --output=type=docker --pull -t $(BUILDER_IMAGE) . ; \
...
ifneq ($(strip $(BUILDER_IMAGE_CACHED)),)
	...
	rm -rf .go _output builder-env-cache
